...
import static org.cssig.kb.gui.ToolBar.ttbbll;
...
//        for (int i=0; i<hSchema.length; i++)    constTypes.add(hSchema[i]);
        ...
        try {
            fn = FNManager.main("To choose the JSON file with data!", ".json");
            if (fn.indexOf(inPath) == -1) {
                inPath = "\\";
            }
...
//			System.out.println("q_neurons: "+q_neurons);
    ...
    private static int getTypePosition(String strPos) {
        ...
        for (int i = 0; i < jTableThingPlenty.getRowCount(); i++) {
            if (jTableThingPlenty.getModel().getValueAt(i, 1).toString().equals(strPos)) {
                pos = i;
            }
        }
        if (pos == -1) {
            JOptionPane.showMessageDialog(null, "Incorrect name of relation !?");
        }
        return pos;
    }
...
//        }
    ...
    public void getScripts() {
        try {
            ...
            File file = new File(FNManager.main("Get data file *json", ".json"));
...
//            ((ArrayNode) contactNode).add(email);
        ...
        try {
            ...
            FileWriter file = new FileWriter(FNManager.main("Save script! .json", ".json"));
...
//        writeFile();
    ...
    public static void writeFile() {
        int i = 0;
        try {
            File file = new File(FNManager.main("To check file to save generator results! .json", ".json"));
            if (file.exists()) {
                file.delete();
            }
            if (file.createNewFile()) {
                BufferedWriter out = new BufferedWriter(new FileWriter(FManager.main("To choose file to save generator results!")));
