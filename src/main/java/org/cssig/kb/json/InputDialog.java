...
//		writeAllFile();
    ...
    public void toAddNewDialog() {
        ...
        for (int i = 0; i < iList.size(); i++) {
            if (iListAll.indexOf(iList.get(i).toString()) == -1) {
                iListAll.add(iList.get(i).toString());
            }
        }

        for (int i = 0; i < oList.size(); i++) {
            if (oListAll.indexOf(oList.get(i).toString()) == -1) {
                oListAll.add(oList.get(i).toString());
            }
        }
...
//    	if((id.contains("<<")) && (!id.contains(">!>"))) oList.add(id.substring(3, id.length()));
        if (id.contains(">>")) {
            iList.add(id.substring(3, id.length()));
        }
        if (id.contains("<<")) {
            oList.add(id.substring(3, id.length()));
        }
        gList.add(id);
    }
...
//        System.out.println(result);
    ...
    // To read file by FileNameManager
    //===========================================
    public void readFile() {
        fileStr = FNManager.main("Open file with dialog .xml", ".xml");
...
//		        System.out.println(textLine);
    ...
    public void writeFile() {
        fileStr = FNManager.main("Save the assessments to file .json", ".json");
